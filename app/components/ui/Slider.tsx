...
export const Slider = genericMemo(
  <T,>({ selected, options, setSelected }: SliderProps<T>) => {
    ...
    const isLeftSelected = hasMiddle
      ? selected === options.left.value
      : selected === options.left.value;
    const isMiddleSelected =
      hasMiddle && options.middle ? selected === options.middle.value : false;

    return (
      <div className="flex items-center flex-wrap shrink-0 gap-1 bg-codinit-elements-background-depth-1 overflow-hidden rounded-full p-1">
        <SliderButton
      ...
      <button
        ...
        className={classNames(
          ...
          selected
            ...
            : "text-codinit-elements-item-contentDefault hover:text-codinit-elements-item-contentActive",
        )}
      >
        ...
        {selected && (
          <motion.span
            ...
            className="absolute inset-0 z-0 bg-codinit-elements-item-backgroundAccent rounded-full"
          ></motion.span>
        )}
      </button>
    );
