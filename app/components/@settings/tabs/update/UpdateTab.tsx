    ...
    {progress.details && (
      <div className="mt-2 text-sm text-gray-600">
        {progress.details.changedFiles &&
          progress.details.changedFiles.length > 0 && (
            <div className="mt-4">
              ...
              <div className="space-y-2">
                ...
                {["Modified", "Added", "Deleted"].map((type) => {
                  const filesOfType =
                    progress.details?.changedFiles?.filter((file) =>
        ...
        {progress.details.totalSize && (
          <div className="mt-1">Total size: {progress.details.totalSize}</div>
        )}
        {progress.details.additions !== undefined &&
          progress.details.deletions !== undefined && (
            <div className="mt-1">
              Changes:{" "}
              <span className="text-green-600">
                +{progress.details.additions}
              </span>{" "}
        ...
        {progress.details.currentCommit && progress.details.remoteCommit && (
          <div className="mt-1">
            Updating from {progress.details.currentCommit} to{" "}
            {progress.details.remoteCommit}
          </div>
        )}
      </div>
    )}
          ...
          try {
            ...
            if (progress.stage === "complete") {
              ...
              if (!progress.error) {
                ...
                if (
                  progress.details?.changedFiles?.length &&
                  progress.details.updateReady
                ) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm text-codinit-elements-textPrimary">
                Automatic Updates
              </span>
      ...
      >
        ...
        {updateProgress?.details?.currentCommit &&
          updateProgress?.details?.remoteCommit && (
            <div className="mt-4 text-sm text-codinit-elements-textSecondary">
              <div className="flex items-center justify-between">
                <div>
                  <p>
                    Updates are fetched from:{" "}
                    <span className="font-mono">Gerome-Elassaad/CodingIT</span>{" "}
        ...
        {updateProgress?.details?.commitMessages &&
          updateProgress.details.commitMessages.length > 0 && (
            <div className="mb-6">
              ...
              <div className="bg-[#F5F5F5] dark:bg-[#1A1A1A] rounded-lg p-4 overflow-auto max-h-[400px]">
                <div className="prose dark:prose-invert prose-sm max-w-none">
                  {updateProgress.details.commitMessages.map(
                    (section, index) => (
                    ...
                    <div className="bg-[#F5F5F5] dark:bg-[#1A1A1A] rounded-lg p-3 space-y-2">
                      {updateProgress.details.commitMessages.map(
                        (msg, index) => (
                          <div
                            key={index}
                            className="text-sm text-codinit-elements-textSecondary flex items-start gap-2"
                          >
