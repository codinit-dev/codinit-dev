      ...
      return (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            <span
              className={
                details.method === "GET" ? "text-green-500" : "text-blue-500"
              }
            >
              {details.method}
            </span>
...
export function EventLogsTab() {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return allLogs.filter((log) => {
      const matchesType =
        log.category === selectedLevel || log.level === selectedLevel;
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !levelFilterRef.current.contains(event.target as Node)
      ) {
        setShowLevelFilter(false);
      }
    };
  ...
  const selectedLevelOption = logLevelOptions.find(
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `codinit-event-logs-${new Date().toISOString()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with codinit.dev
      doc.setFontSize(12);
      doc.setFont("helvetica", "normal");
      doc.text("codinit.dev - AI Development Platform", margin, 45);
      yPos = 70;
      ...
      const stats = {
        ...
        provider: filteredLogs.filter((log) => log.category === "provider")
          .length,
      ...
      const addLogEntry = (log: LogEntry) => {
        ...
        if (log.category) {
          const categoryX =
            margin + levelWidth + doc.getTextWidth(timestamp) + 20;
          doc.setFillColor("#F3F4F6");
        ...
        if (log.details) {
          ...
          doc.roundedRect(
            ...
            yPos - 2,
            maxLineWidth - 10,
            detailsLines.length * lineHeight + 8,
            1,
            1,
            "F",
          );
      ...
      // Save the PDF
      doc.save(`codinit-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `codinit-event-logs-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
    ...
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
        <DropdownMenu.Root
          ...
          onOpenChange={setShowLevelFilter}
        >
          <DropdownMenu.Trigger asChild>
            ...
            >
              <span
                className={classNames(
                  "text-lg",
                  selectedLevelOption?.icon || "i-ph:funnel",
                )}
                ...
                >
                  <div className="mr-3 flex h-5 w-5 items-center justify-center">
                    <div
                      className={classNames(
                        option.icon,
                        "text-lg group-hover:text-purple-500 transition-colors",
                      )}
