  ...
  async checkStatus(): Promise<StatusCheckResult> {
    try {
      ...
      const lastUpdate = lastUpdateMatch
        ? `${lastUpdateMatch[1]}${lastUpdateMatch[2]}`
        : "";
      ...
      const services = {
        "Huggingface Hub": {
          operational:
            text.includes("Huggingface Hub") && text.includes("Operational"),
          uptime: text.match(
            /Huggingface Hub[\s\S]*?(\d+\.\d+)%\s*uptime/,
          )?.[1],
        },
        "Git Hosting and Serving": {
          ...
          uptime: text.match(
            /Git Hosting and Serving[\s\S]*?(\d+\.\d+)%\s*uptime/,
          )?.[1],
        },
        "Inference API": {
          operational:
            text.includes("Inference API") && text.includes("Operational"),
          uptime: text.match(/Inference API[\s\S]*?(\d+\.\d+)%\s*uptime/)?.[1],
        },
        "HF Endpoints": {
          operational:
            text.includes("HF Endpoints") && text.includes("Operational"),
