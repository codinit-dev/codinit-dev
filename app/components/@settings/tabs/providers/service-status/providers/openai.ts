    ...
    try {
      ...
      const services = {
        api: {
          ...
          outage:
            text.includes("API ?  Major Outage") ||
            text.includes("API ?  Partial Outage"),
        },
        chat: {
          operational: text.includes("ChatGPT ?  Operational"),
          degraded: text.includes("ChatGPT ?  Degraded Performance"),
          outage:
            text.includes("ChatGPT ?  Major Outage") ||
            text.includes("ChatGPT ?  Partial Outage"),
        },
      };

      // Extract recent incidents
      const incidents: string[] = [];
      const incidentMatches = text.match(
        /Past Incidents(.*?)(?=\w+ \d+, \d{4})/s,
      );
