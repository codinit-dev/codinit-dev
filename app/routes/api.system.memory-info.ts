  ...
  try {
    ...
    if (platform === "darwin") {
      ...
      const totalMemory = parseInt(
        execSync("sysctl -n hw.memsize").toString().trim(),
        10,
      );
      ...
      const usedMemory =
        (activePages + inactivePages + wiredPages + compressedPages) * pageSize;
      ...
      try {
        ...
        const swapTotal =
          parseFloat(swapMatches.total?.[1] || "0") * 1024 * 1024;
      ...
      const memTotal =
        parseInt(/MemTotal:\s+(\d+)/.exec(meminfo)?.[1] || "0", 10) * 1024;

      // We use memAvailable instead of memFree for more accurate free memory calculation
      const memAvailable =
        parseInt(/MemAvailable:\s+(\d+)/.exec(meminfo)?.[1] || "0", 10) * 1024;
      ...
      // Get swap information
      const swapTotal =
        parseInt(/SwapTotal:\s+(\d+)/.exec(meminfo)?.[1] || "0", 10) * 1024;
      const swapFree =
        parseInt(/SwapFree:\s+(\d+)/.exec(meminfo)?.[1] || "0", 10) * 1024;
      ...
      try {
        ...
        const swapTotal =
          parseInt(swapData.AllocatedBaseSize, 10) * 1024 * 1024;
