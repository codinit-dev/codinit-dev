...
async function chatAction({ context, request }: ActionFunctionArgs) {
  const { messages, files, promptId, contextOptimization, supabase } =
    await request.json<{
      messages: Messages;
      files: any;
      promptId?: string;
  ...
  try {
    ...
    logger.debug(
      `Total message length: ${totalMessageContent.split(" ").length}, words`,
    );
    ...
    const dataStream = createDataStream({
      async execute(dataStream) {
        ...
        if (filePaths.length > 0 && contextOptimization) {
          ...
          if (filteredFiles) {
            logger.debug(
              `files in context : ${JSON.stringify(Object.keys(filteredFiles))}`,
            );
          }
        ...
        transform: (chunk, controller) => {
          ...
          if (typeof chunk === "string") {
            if (chunk.startsWith("g") && !lastChunk.startsWith("g")) {
              controller.enqueue(
                encoder.encode(`0: "<div class=\\"__codinitThought__\\">"\n`),
              );
            }
          ...
          const str =
            typeof transformedChunk === "string"
              ? transformedChunk
              : JSON.stringify(transformedChunk);
