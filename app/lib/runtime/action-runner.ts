...
import { path as nodePath } from 'codinit-agent/utils/path';
import { atom, map, type MapStore, type WritableAtom } from 'nanostores';
import type { ActionAlert, FileHistory } from '~/types/actions';
import { createScopedLogger } from 'codinit-agent/utils/logger';
import { unreachable } from 'codinit-agent/utils/unreachable';
...
import { npmInstallToolParameters } from 'codinit-agent/tools/npmInstall';
import { workbenchStore } from '~/lib/stores/workbench';
import { z } from 'zod';
import { editToolParameters } from 'codinit-agent/tools/edit';
import { getAbsolutePath } from 'codinit-agent/utils/workDir';
import { cleanCodinitOutput } from 'codinit-agent/utils/shell';
import type { CodinitAction } from 'codinit-agent/types';
import type { ExampleShell } from '~/utils/shell';
import { streamOutput } from '~/utils/process';
import { outputLabels } from '~/lib/runtime/deployToolOutputLabels';
import type { CodinitToolName } from 'codinit-agent/types';
import { lookupDocsParameters, docs, type DocKey } from 'codinit-agent/tools/lookupDocs';
import { addEnvironmentVariablesParameters } from 'codinit-agent/tools/addEnvironmentVariables';
import { openDashboardToPath } from '~/lib/stores/dashboardPath';
import { codinitProjectStore } from '~/lib/stores/codinitProject';

const logger = createScopedLogger('ActionRunner');

export type ActionStatus = 'pending' | 'running' | 'complete' | 'aborted' | 'failed';

type BaseActionState = CodinitAction & {
...
type FailedActionState = CodinitAction &
...
type ActionStateUpdate =
  ...
  | Pick<BaseActionState & { type: 'codinit' }, 'output'>;
      ...
      if (parsed.state === 'call') {
        ...
        if (previousCall) {
          this.onToolCallComplete({
            ...
            toolName: parsed.toolName as CodinitToolName,
          });
          return;
        }
    ...
    try {
      switch (parsed.toolName) {
        ...
        case 'deploy': {
          ...
          const abortSignal = action.abortSignal;
          ...
          const runCodegenAndTypecheck = async (onOutput?: (output: string) => void) => {
            ...
            let output = await run(['codinit', 'codegen'], outputLabels.codinitTypecheck, onOutput);
            output += await run(
          ...
          if (!workbenchStore.isDefaultPreviewRunning()) {
            await this.#shellTerminal.executeCommand('start-preview', 'vite --open');
