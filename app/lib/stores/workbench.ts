...
export class WorkbenchStore {
  ...
  actionAlert: WritableAtom<ActionAlert | undefined> =
    import.meta.hot?.data.actionAlert ??
    atom<ActionAlert | undefined>(undefined);
  supabaseAlert: WritableAtom<SupabaseAlert | undefined> =
    import.meta.hot?.data.supabaseAlert ??
    atom<SupabaseAlert | undefined>(undefined);
  deployAlert: WritableAtom<DeployAlert | undefined> =
    import.meta.hot?.data.deployAlert ??
    atom<DeployAlert | undefined>(undefined);
  ...
  async deleteFolder(folderPath: string) {
    try {
      const currentDocument = this.currentDocument.get();
      const isInCurrentFolder = currentDocument?.filePath?.startsWith(
        folderPath + "/",
      );
  ...
  async pushToGitHub(
    ...
    try {
      ...
      if (!githubToken || !owner) {
        throw new Error(
          "GitHub token or username is not set in cookies or provided.",
        );
      }
      ...
      try {
        ...
        if (
          ...
          const { data: newRepo } =
            await octokit.repos.createForAuthenticatedUser(createRepoOptions);
      ...
      const pushFilesToRepo = async (attempt = 1): Promise<string> => {
        ...
        try {
          console.log(
            `Pushing files to repository (attempt ${attempt}/${maxAttempts})...`,
          );
          ...
          const repoRefresh = await octokit.repos.get({
            owner,
            repo: repoName,
          });
          ...
          if (
            (visibilityJustChanged || attempt === 1) &&
            attempt < maxAttempts
          ) {
