...
import { ROLE_SYSTEM_PROMPT, generalSystemPrompt } from 'codinit-agent/prompts/system';
import { deployTool } from 'codinit-agent/tools/deploy';
import { viewTool } from 'codinit-agent/tools/view';
import type { CodinitToolSet } from 'codinit-agent/types';
import { npmInstallTool } from 'codinit-agent/tools/npmInstall';
import type { Tracer } from '~/lib/.server/chat';
import { editTool } from 'codinit-agent/tools/edit';
import { captureException, captureMessage } from '@sentry/remix';
import type { SystemPromptOptions } from 'codinit-agent/types';
...
import { getCodinitSiteUrl } from '~/lib/codinitSiteUrl';
import { REPEATED_ERROR_REASON } from '~/lib/common/annotations';
import { waitUntil } from '@vercel/functions';
import type { internal } from '@codinit/_generated/api';
import type { Usage } from '~/lib/common/annotations';
import type { UsageRecord } from '@codinit/schema';
import { getProvider, type ModelProvider } from '~/lib/.server/llm/provider';
import { getEnv } from '~/lib/.server/env';
import { calculateCodinitTokens, usageFromGeneration } from '~/lib/common/usage';
import { lookupDocsTool } from 'codinit-agent/tools/lookupDocs';
import { addEnvironmentVariablesTool } from 'codinit-agent/tools/addEnvironmentVariables';
import { getCodinitDeploymentNameTool } from 'codinit-agent/tools/getCodinitDeploymentName';
import type { PromptCharacterCounts } from 'codinit-agent/ChatContextManager';

type Messages = Message[];

export async function codinitAgent(args: {
...
/* Convert Usage into something stable to store in CodinIT debug logs */
...
async function storeDebugPrompt(
  ...
  try {
    ...
    const { codinitTokens } = calculateCodinitTokens(usage, modelProvider);
    ...
    const response = await fetch(`${getCodinitSiteUrl()}/upload_debug_prompt`, {
      method: 'POST',
      body: formData,
    });
