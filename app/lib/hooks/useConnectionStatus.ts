...
const ACKNOWLEDGED_CONNECTION_ISSUE_KEY =
...
export const useConnectionStatus = () => {
  ...
  const [acknowledgedIssue, setAcknowledgedIssue] = useState<string | null>(
    () => getAcknowledgedIssue(),
  );

  const checkStatus = async () => {
    try {
      const status = await checkConnection();
      const issue = !status.connected
        ? "disconnected"
        : status.latency > 1000
          ? "high-latency"
          : null;
