...
describe('ChatContextManager', () => {
  ...
  const relevantFilesMessage = createMessage('user', [
    {
      type: 'text',
      text: `<codinitArtifact id="1" title="Relevant Files">
<codinitAction type="file" filePath="/home/project/package.json">{"name": "test"}</codinitAction>
</codinitArtifact>`,
    },
  ]);
  const emptyRelevantFilesMessage = createMessage('user', [
    {
      type: 'text',
      text: `<codinitArtifact id="1" title="Relevant Files">
<codinitAction type="file" filePath="/home/project/package.json"></codinitAction>
</codinitArtifact>`,
    },
  ]);
...
<codinitAction type="file" filePath="/home/project/package.json"></codinitAction>
    ...
    test('returns true when previous message has Relevant Files but no codinitAction', () => {
      const manager = createManager();
      const messages = [
        createMessage('user', [
          {
    ...
    test('returns true when previous message has multiple empty codinitActions', () => {
      const manager = createManager();
      const messages = [emptyRelevantFilesMessage];
      expect(manager.shouldSendRelevantFiles(messages, maxCollapsedMessagesSize)).toBe(true);
    });

    test('returns false when previous message has at least one non-empty codinitAction', () => {
      const manager = createManager();
