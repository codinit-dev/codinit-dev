...
export class ChatContextManager {
   ...
   *
   * 1. The CodinIT guidelines, which are filled in by the server and
  ...
  relevantFiles(messages: UIMessage[], id: string, maxRelevantFilesSize: number): UIMessage {
    ...
    for (const [path] of sortedByLastUsed) {
      ...
      if (entry.type === 'file') {
        ...
        fileActions.push(`<codinitAction type="file" filePath="${path}">${content}</codinitAction>`);
    ...
    if (currentDocument) {
      ...
      fileActions.push(`<codinitAction type="file" filePath="${currentDocument.filePath}">${content}</codinitAction>`);
    }
      ...
      if (message.role === 'user') {
        for (const part of message.parts) {
          if (part.type === 'text' && part.text.includes('title="Relevant Files"')) {
            // Check if there's actual content between the codinitAction tags
            // We used to strip out the file content when serializing messages to store in CodinIT
            const hasContent =
              part.text.includes('<codinitAction type="file"') && !part.text.match(/<codinitAction[^>]*><\/codinitAction>/);
            if (hasContent) {
...
function makeUserMessage(content: string[], id: string): UIMessage {
  const parts: UIMessagePart[] = content.map((c) => ({
    ...
    text: `<codinitArtifact id="${id}" title="Relevant Files">
${c}
</codinitArtifact>`,
  }));
  return {
...
function abbreviateToolInvocation(toolInvocation: ToolInvocation): string {
  ...
  switch (toolInvocation.toolName) {
    ...
    case 'getCodinitDeploymentName': {
      toolCall = `retrieved the CodinIT deployment name`;
      break;
    }
    default:
