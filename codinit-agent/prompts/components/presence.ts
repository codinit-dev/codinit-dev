...
# CodinIT PresenceComponent

A CodinIT component for managing presence functionality, i.e., a live-updating
...
component implements presence via CodinIT scheduled functions such that clients
...
\`\`\`bash
npm install @codinit-dev/presence
\`\`\`

## Usage

First, add the component to your CodinIT app:

\`codinit/codinit.config.ts\`

\`\`\`ts
import { defineApp } from "codinit/server";
import presence from "@codinit-dev/presence/codinit.config";
...
import { Presence } from "@codinit-dev/presence";
...
import usePresence from "@codinit-dev/presence/react";
import FacePile from "@codinit-dev/presence/facepile";
