...
# Resend CodinIT Component (Beta)

[![npm version](https://badge.fury.io/js/@codinit-dev%2Fresend.svg)](https://badge.fury.io/js/@codinit-dev%2Fresend)

This component is the official way to integrate the Resend email service
with your CodinIT project.
...
- Durable execution: Uses CodinIT workpools to ensure emails are eventually delivered, even in the face of temporary failures or network outages.
...
\`\`\`bash
npm install @codinit-dev/resend
\`\`\`
...
Next, add the component to your CodinIT app via \`codinit/codinit.config.ts\`:

\`\`\`ts
import { defineApp } from "codinit/server";
import resend from "@codinit-dev/resend/codinit.config";
...
Then you can use it, as we see in \`codinit/sendEmails.ts\`:

\`\`\`ts
import { components } from "./_generated/api";
import { Resend } from "@codinit-dev/resend";
...
On the CodinITside, we need to mount an http endpoint to our project to route it to
the Resend component in \`codinit/http.ts\`:
...
If our CodinITdeployment is happy-leopard-123, we now have an API for a Resend webhook at
\`https://happy-leopard-123.codinit.site/resend-webhook\`.
Use the getcodinitDeploymentName tool to get the deployment name and print the correct URL for the user to copy and paste.
...
import { Resend } from "@codinit-dev/resend";
import { internalMutation } from "./_generated/server";
import { vEmailId, vEmailEvent, Resend } from "@codinit-dev/resend";
