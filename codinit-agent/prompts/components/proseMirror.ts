...
# codinit ProseMirror Component

[![npm version](https://badge.fury.io/js/@codinit-dev%2Fprosemirror-sync.svg)](https://badge.fury.io/js/@codinit-dev%2Fprosemirror-sync)

This is a [codinit Component](https://codinit.dev/components) that syncs a
...
synced to the cloud. The data lives in your codinit database, and can be stored
alongside the rest of your app's data.

Just configure your editor features, add this component to your codinit backend,
...
\`\`\`ts
npm install @codinit-dev/prosemirror-sync
\`\`\`

Create a \`codinit.config.ts\` file in your app's \`codinit/\` folder and install the component by calling \`use\`:

\`\`\`ts
// codinit/codinit.config.ts
import { defineApp } from 'codinit/server';
import prosemirrorSync from '@codinit-dev/prosemirror-sync/codinit.config';
...
To use the component, you expose the API in a file in your \`codinit/\` folder, and
...
import { ProsemirrorSync } from '@codinit-dev/prosemirror-sync';
...
- If you export these in \`codinit/prosemirror.ts\`, pass \`api.prosemirror\`
...
- import { DataModel } from "./codinit/\_generated/dataModel";
- import { GenericQueryCtx } from 'codinit/server';
- // ...
...
- @returns functions to export, so the \`useBlockNoteSync\` hook can use them.
  ...
  - @param ctx - A codinit query context.
  ...
  - @param ctx - A codinit mutation context.
  - @param id - The document ID.
    \*/
    checkWrite?: (
...
[\`codinitProvider\`](https://docs.codinit.dev/quickstart/react#:~:text=Connect%20the%20app%20to%20your%20backend)
...
\`\`\`tsx
// src/MyComponent.tsx
import { useBlockNoteSync } from '@codinit-dev/prosemirror-sync/blocknote';
...
/**
 * A hook to sync a BlockNote editor with a codinit document.
 *
 * Usually used like:
 *
 * \`\`\`tsx
