...
export function resendProxyGuidelines(options: SystemPromptOptions) {
  ...
  <bundled_resend_proxy_guidelines>
    Apps in the Codinit environment come with a small number of emails
    to send via the Resend API! The Codinit environment ONLY supports
    sending emails to a user's verified email address. For example,
    if a developer signs into Codinit with a GitHub account with email
    test@example.com, their apps built on Codinit can only use the
    CodinIT Resend proxy to send emails to test@example.com. Sending
    to any other email address will result in an error.

    Emails from the Resend proxy will always come from "Codinit Notifications
    <{DEPLOYMENT_NAME}@codinit.app>". The Resend SDK still requires
    a "from" field, however, so put something sensible in there for when
    the user sets up their own Resend API key.

    The environment provides the \`CODINIT_RESEND_API_KEY\` and
    ...
    export const sendReport = action({
      ...
      handler: async (ctx, args) => {
        ...
        const resend = new Resend(process.env.CODINIT_RESEND_API_KEY);
    ...
    \`CODINIT_RESEND_API_KEY\` and \`RESEND_BASE_URL\` environment variables.
    ...
    that over the builtin CodinIT one. You may need to tell them to remove
    the "RESEND_BASE_URL" environment variable to have the Resend SDK not
    use the CodinIT proxy.
  </bundled_resend_proxy_guidelines>
  `;
}
