...
export function codinitGuidelines(options: SystemPromptOptions) {
  return stripIndents`# CodinIT guidelines

## Function guidelines

### New function syntax

- ALWAYS use the new function syntax for CodinIT functions. For example:

\`\`\`ts
import { query } from "./_generated/server";
import { v } from "codinit/values";
...
- HTTP endpoints are defined in \`codinit/http.ts\` and require an \`httpAction\` decorator. For example:

\`\`\`ts
import { httpRouter } from "codinit/server";
...
- Here are the valid CodinIT types along with their respective validators:
 CodinIT Type  | TS/JS type  |  Example Usage         | Validator for argument validation and schemas  | Notes
                                                                              |
| ----------- | ------------| -----------------------| -----------------------------------------------| ------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------|
| Id          | string      | \`doc._id\`              | \`v.id(tableName)\`                              |
                                                                              |
| Null        | null        | \`null\`                 | \`v.null()\`                                     | JavaScript's \`undefined\` is not a valid CodinIT value. Functions the return \`undefined\` or do not return will return \`null\` when called from a client. Use \`null\` instead.                             |
| Int64       | bigint      | \`3n\`                   | \`v.int64()\`                                    | Int64s only support BigInts between -2^63 and 2^63-1. CodinIT supports \`bigint\`s in most modern browsers.
                                                                              |
| Float64     | number      | \`3.1\`                  | \`v.number()\`                                   | CodinIT supports all IEEE-754 double-precision floating point numbers (such as NaNs). Inf and NaN are JSON serialized as
strings.                                                                      |
...
| Bytes       | ArrayBuffer | \`new ArrayBuffer(8)\`   | \`v.bytes()\`                                    | CodinIT supports first class bytestrings, passed in as \`ArrayBuffer\`s. Bytestrings must be smaller than the 1MB total siz
e limit for CodinIT types.                                                     |
| Array       | Array]      | \`[1, 3.2, "abc"]\`      | \`v.array(values)\`                              | Arrays can have at most 8192 values.
                                                                              |
| Object      | Object      | \`{a: "abc"}\`           | \`v.object({property: value})\`                  | CodinIT only supports "plain old JavaScript objects" (objects that do not have a custom prototype). Objects can have at most 1024 entries. Field names must be ASCII characters, nonempty, and not start with "$" or "_". |
...
\`\`\`ts
import { defineSchema, defineTable } from "codinit/server";
import { v } from "codinit/values";
...
- Use \`internalQuery\`, \`internalMutation\`, and \`internalAction\` to register internal functions. These functions are private and aren't part of an app's API. They can only be called by other CodinIT functions. These functions are always imported from \`./_generated/server\`.
- Use \`query\`, \`mutation\`, and \`action\` to register public functions. These functions are part of the public API and are exposed to the public Internet. Do NOT use \`query\`, \`mutation\`, or \`action\` to register sensitive internal functions that should be kept private.
- You CANNOT register a function through the \`api\` or \`internal\` objects.
- ALWAYS include argument validators for all CodinIT functions. This includes all of \`query\`, \`internalQuery\`, \`mutation\`, \`internalMutation\`, \`action\`, and \`internalAction\`.
- If the JavaScript implementation of a CodinIT function doesn't have a return value, it implicitly returns \`null\`.
...
- Function references are pointers to registered CodinIT functions.
- ALWAYS use the \`api\` object defined by the framework in \`codinit/_generated/api.ts\` to call public functions registered with \`query\`, \`mutation\`, or \`action\`. You must import the \`api\` object in the same file when using it and it looks like:

\`\`\`ts
import { api } from "./_generated/api";
\`\`\`

- ALWAYS use the \`internal\` object defined by the framework in \`codinit/_generated/api.ts\` to call internal (or private) functions registered with \`internalQuery\`, \`internalMutation\`, or \`internalAction\`. You must import the \`internal\` object in the same file when using it and it looks like:

\`\`\`ts
import { internal } from "./_generated/api";
\`\`\`

- CodinIT uses file-based routing, so a public function defined in \`codinit/example.ts\` named \`f\` has a function reference of \`api.example.f\`.
- A private function defined in \`codinit/example.ts\` named \`g\` has a function reference of \`internal.example.g\`.
- Functions can also registered within directories nested within the \`codinit/\` folder. For example, a public function \`h\` defined in \`codinit/messages/access.ts\` has a function reference of \`api.messages.access.h\`.

### Api design

- CodinIT uses file-based routing, so thoughtfully organize files with public query, mutation, or action functions within the \`codinit/\` directory.
- Use \`query\`, \`mutation\`, and \`action\` to define public functions.
- Use \`internalQuery\`, \`internalMutation\`, and \`internalAction\` to define private, internal functions.

### Limits

To keep performance fast, CodinIT puts limits on function calls and database records:
...
- Objects in function arguments and return values must be valid CodinIT objects, so they can
  only contain ASCII field names. ALWAYS remap non-ASCII characters like emoji to an
  ASCII code before storing them in an object synced to codinit.
...
CodinIT supports environment variables within function calls via \`process.env\`. Environment
...
import { paginationOptsValidator } from "codinit/server";
...
- Always define your schema in \`codinit/schema.ts\`.
- Always import the schema definition functions from \`codinit/server\`:
...
- The system provides two built-in indexes: "by_id" and "by_creation_time." Never add these to the
  ...
  is ALWAYS wrong.
- CodinIT automatically includes \`_creationTime\` as the final column in all indexes.
...
Path: \`codinit/schema.ts\`
\`\`\`ts
import { defineSchema } from "codinit/server";
...
- CodinIT queries do NOT support \`.delete()\`. Instead, \`.collect()\` the results, iterate over them, and call \`ctx.db.delete(row._id)\` on each result.
- Use \`.unique()\` to get a single document from a query. This method will throw an error if there are multiple documents that match the query.
- When using async iteration, don't use \`.collect()\` or \`.take(n)\` on the result of a query. Instead, use the \`for await (const row of query)\` syntax.

### Ordering

- By default CodinIT always returns documents in ascending \`_creationTime\` order.
...
\`\`\`ts
import { cronJobs } from "codinit/server";
import { internal } from "./_generated/api";
...
- You can register CodinIT functions within \`crons.ts\` just like any other file.
...
- CodinIT includes file storage for large files like images, videos, and PDFs.
...
- Images are stored as CodinIT storage IDs. Do NOT directly as image URLs. Instead, fetch the signed URL for each image from codinit
...
- CodinIT storage stores items as \`Blob\` objects. You must convert all items to/from a \`Blob\` when using CodinIT storage.

# Examples
## Example of using CodinIT storage within a chat app

This example creates a mutation to generate a short-lived upload URL and a mutation to save an image message to the database. This mutation is called from the client, which uses the generated upload URL to upload an image to CodinIT storage. Then,
...
import { defineSchema, defineTable } from "codinit/server";
import { v } from "codinit/values";
import { authTables } from "@codinit-dev/auth/server";
...
# CodinIT Components
CodinIT Components package up code and data in a sandbox that allows you to confidently and quickly add new features to your backend.
CodinIT Components are like mini self-contained CodinIT backends, and installing them is always safe. They can't read your app's tables or call your app's functions unless you pass them in explicitly.
Each component is installed as its own independent library from NPM. You also need to add a \`codinit.config.ts\` file that includes the component.
...
CodinIT has the following components:
...
CodinIT has but does not support the following components in Codinit: 
DO NOT use the \`lookupDocs\` tool to lookup documentation for these or install them.
Codinit does not have documentation for them. Tell the user that they are unsupported now but will be supported in the future.
- Workflow
- AI Agent
