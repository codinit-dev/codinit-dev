...
import { SUGGESTIONS } from 'codinit-agent/constants.js';
...
import { codinitScorer } from './codinitScorer.js';
import type { CodinitModel } from './types.js';
import * as net from 'net';

const CHEF_PROJECT = 'codinit';

function codinitEval(model: CodinitModel) {
  const experimentName = `${CHEF_PROJECT}-${model.name}`;
  let outputDir = process.env.OUTPUT_TEMPDIR;
  if (!outputDir) {
    outputDir = mkdtempSync(path.join(os.tmpdir(), 'codinit-eval'));
  }
  const environment = process.env.ENVIRONMENT ?? 'dev';
  return braintrust.Eval(CHEF_PROJECT, {
    experimentName,
    data: SUGGESTIONS.map((s) => ({ input: s.prompt })),
    task: (input: string) => codinitTask(model, outputDir!, input),
    scores: [codinitScorer],
    maxConcurrency: 2,
    metadata: {
      model: model.name,
